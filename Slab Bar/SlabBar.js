...
// const singleLabelRegex = /^\s*(?:([A-Za-z]+)\s*[-]?\s*)?(?:([YTOØ]|TOR|#)\s*)?(\d+)\s*(?:@\s*(\d+)\s*(?:c\/c|" c\/c))?\s*$/;
const singleLabelRegex = /^\s*(?:([A-Za-z]+)\s*-?\s*)?(TOR|[YTOØ#])\s*(\d+)(?:\s*@\s*(\d+)(?:\s*(?:c\/c|C\/C|"?c\/c"?))?)?\s*$/i;
// have to discuss this regex 
